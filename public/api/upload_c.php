...
class FileUpload {
    ...
    public function uploadLeadData($dbio, $requestObject){
        ...
        if($fileError == UPLOAD_ERR_OK){
            ...
            $dbconn = $dbio->getConn();
            for($i=2; $i<=$highestRow; $i++){ // looping for every row that exist in excel.
                if($leadsrc == "4"){
                    for($x=2; $x<=13; $x++){ // looping for each column in a row that exist in excel.
                        $datalist["C".$x] =  $sheet->getCellByColumnAndRow($x, $i)->getValue();
                    }
                    $branName = str_replace("Zenith Leisure Holidays-","",$datalist["C2"]);
                    $branchCode = $this->getBranchCode($dbio,$dbconn, $branName);   // to get location_code from location_name if exist, otherwise will throw error.
                    $orderno = $datalist["C3"];
                    $isd = $datalist["C4"];
                    $isdcode = $datalist["C5"];
                    ...
                    $branchCode = $this->getBranchCode($dbio,$dbconn, $branName[1]);    // to get location_code from location_name if exist, otherwise will throw error.
                    $orderno = $datalist["C2"];
                ...
                $isIsdExist = $this->getIsdCode($dbio,$dbconn, $isdcode);       // to get isd code from isd_name if exist, otherwise will throw error.
                $isOrderExist = $this->checkOrderExist($dbio,$dbconn, $orderno); // to check if order already exist in table
                
                $ordernoArr[] = $orderno;
                $countOrder = array_count_values($ordernoArr); // to check if more than one product exist with same order number
                ...
                if($isRepeated == 0){
                    ...
                    $qry = $qry."INSERT INTO lead_order (po_srno, po_leadsource, po_ordertype, po_productcount, po_product, po_product_2, po_currency, po_card_currency, 
                            ...
                            '', '', 1, ".$_SESSION["userSrno"]." ";
                        if($status == 14) $qry = $qry." , NOW();"; // done leads
                        else $qry = $qry." , NULL;"; // pending leads
            ...
            if($isOk){
                $dbconn = $dbio->getConn();
                    $result = $dbio->batchQueries($dbconn, $qry); // for lead_order (main table)
                $dbio->closeConn($dbconn);
                if($result){
                    $dbconn = $dbio->getConn();
                        $result = $dbio->batchQueries($dbconn, $qry_1); // master_account_details (account details table)
                    $dbio->closeConn($dbconn);
                    $dbconn = $dbio->getConn();
                        $result = $dbio->batchQueries($dbconn, $qry_3); // lead_product (product details table)
                    $dbio->closeConn($dbconn);
                    $dbconn = $dbio->getConn();
                        $result = $dbio->batchQueries($dbconn, $qry_2); // lead_traveller (traveller detail table)
                    $dbio->closeConn($dbconn);
                    $dbconn = $dbio->getConn();
                        $result = $dbio->batchQueries($dbconn, $qry_4); // mis_product (table for showing product in opportunity menu)
                    $dbio->closeConn($dbconn);
                    if($qry_5 != ""){
                        $dbconn = $dbio->getConn();
                            $result = $dbio->batchQueries($dbconn, $qry_5); // updating main table if more than one product exist in uploaded excel
                        $dbio->closeConn($dbconn);
                    }
                }
                if($result){
                    ...
                    $this->msg = 2; //Error during query execution
                }
            }else{
                $this->msg = 0;
         ...
         $sql = "";
            ...
            if($result){
                while($row = mysqli_fetch_assoc($result)){
                    $isdlist[] = $row['value']; // Store each 'md_code' value into the array
                }
            }
            ...
            if(mysqli_num_rows($result1) > 0){
                while($row = mysqli_fetch_assoc($result1)){
                    $branlist[] = $row['cu_branchcd']; // Store each 'md_code' value into the array
                }
            }
            ...
            if(mysqli_num_rows($result1) > 0){
                while($row = mysqli_fetch_assoc($result1)){
                    $usercorpsrno = $row['user_corpsrno']; // Store each 'md_code' value into the array
                }
            }
