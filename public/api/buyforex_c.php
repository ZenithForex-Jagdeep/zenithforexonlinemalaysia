...
class Buyforex
    ...
    private function sendSeparateMail( $title, $curtime, $ml, $email )
 {
        ...
        //     // return $msent;
        //     exit();
        // Child process exits after sending email
        // }
        require_once( 'mail_c.php' );
        $m = new Mymail();
        $msent = $m->sendMail( $title . $curtime . '', $ml, $email, '', '', '' );
    ...
    function calculateTcs( $dbio, $panNumber, $totalAmt, $purpose, $educationLoan, $handlingcharge = 0, $gst = 0 )
 {
        ...
        //, COALESCE( SUM( po_handlingcharge ), 0 ), COALESCE( SUM( po_CGST+po_SGST+po_IGST ), 0 )
        $isok = true;
        $earlierForex = 0;
        //calculate earlier forex from zenithglobal.com.my
    ...
    public function submitLeadHeader( $dbio, $obj )
 {
        ...
        $id = $obj->id;
        $sourceOfFund = $obj->sourceOfFund;
        // this should be S or L.
        $itr = $obj->itr;
        // this should be Y or N.
