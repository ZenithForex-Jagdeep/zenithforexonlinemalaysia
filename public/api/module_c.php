<?php
    ...
    class CorpModule {
        ...
        public function getCorpData($dbio, $product){
            ...
            if(mysqli_num_rows($res)>0){
                ...
                }else if($row[0]*1 === 2){//for live rate
                    $this->isliverate = $row[0];
        ...
        public function saveLeadDetail($dbio, $obj){
            ...
            if(mysqli_num_rows($res)>0){
                ...
                if(LIVE){
                    $url = "https://www.zenithforexonline.com/corporate_upload?data=" . $queryParam;
                }else{
        ...
        public function viewLOA($dbio, $obj){
            ...
            if(!file_exists(ENTITYDOCSPATH.$doc_filename)){
                ...
                if($doc_ext=="jpg"){
                    ...
                    $typ = "image/png";
                    $bs64 = "data:image/png;base64,".base64_encode(file_get_contents(ENTITYDOCSPATH."fnf.png")); /// file not found file not available
                }              
            }
    ...
    public function errorUploadData($dbio, $obj) {
        $tableData = $obj; // Assuming $obj is an array of rows (associative arrays)
        $format = "<html>";
