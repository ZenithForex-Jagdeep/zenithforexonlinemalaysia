...
class User
    ...
    public function validateLogin( $dbio, $username, $password, $appname )
 {
        if ( strlen( $password ) > 0 and strlen( $username ) > 0 ) {
            ...
            if ( mysqli_num_rows( $result ) > 0 ) {
                ...
                if ( $row[ 4 ] == $dbio->encryptString( $password ) ) {
                    //correct password
                    if ( $row[ 5 ] != 1 || ( int ) $row[ 13 ] >= 5 ) {
                        //user inactive and invalid attempts more than 5 times.
                        $this->status = 'MSG0003';
                    } elseif ( $row[ 6 ] != 1 ) {
                        //user disable
                        $this->status = 'MSG0004';
                    ...
                    // invalid password
                    if ( ( int ) $row[ 13 ] < 5 ) {
                        //update invalide user column in the user_login table
                        ...
                        $result = $dbio->getSelect( $dbconn, $sql );
                        $dbio->closeConn( $dbconn );
                    } else if ( ( ( int ) $row[ 5 ] == 1 ) && ( ( int ) $row[ 13 ] > 5 ) ) {
                        //if wrong pass greater than 5 times than user should be invalid
    ...
    public function verifyOtp( $dbio, $mobile, $email )
 {
        ...
        if ( $row[ 4 ] == 1 ) {
            ...
            $this->status = 'phone0';
            //phone and email otp not verfied
        }
    }
    ...
    public function resetPass( $dbio, $email )
 {
        ...
        if ( mysqli_affected_rows( $dbconn ) > 0 ) {
            ...
            $ml = $ml . 'Dear ' . $row[ 1 ] . '<br><br> New password for your login is <b>' . $newPass . "</b> <br>
                    ...
                    Link: https://www.zenithglobal.com.my/login <br><br> <b>Thanks & Regards <br> Administrator </b>";
            $ml = $ml . '</body>';
            $ml = $ml . '</html>';
            require_once( 'mail_c.php' );
            $m = new Mymail();
